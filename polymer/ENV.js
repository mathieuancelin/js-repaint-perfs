var t = document.querySelector('db-table');
...
var ENV = ENV || (function() {
  ...
  function generateRow(object, keepIdentity, counter, row) {
    ...
    var lastSample = `databases.${row}.lastSample`;

    for (var i = 0; i < 5; i++) {
      var source = object.lastSample.queries[i];
      object.lastSample.topFiveQueries[i] = source;

      // Notify Polymer of this data change.
      t.notifyPath(`${lastSample}.topFiveQueries.${i}.elapsedClassName`, source.elapsedClassName);
      t.notifyPath(`${lastSample}.topFiveQueries.${i}.formatElapsed`, source.formatElapsed);
      t.notifyPath(`${lastSample}.topFiveQueries.${i}.query`, source.query);
    }

    object.lastSample.nbQueries = nbQueries;

    // Notify Polymer of this data change.
    t.notifyPath(`${lastSample}.nbQueries`, nbQueries);

    object.lastSample.countClassName = countClassName(nbQueries);

    // Notify Polymer of this data change.
    t.notifyPath(`${lastSample}.countClassName`, countClassName(nbQueries));
  ...
  function getData(keepIdentity) {
    ...
    for (var i in data) {
      ...
      if (!row.lastSample || Math.random() < ENV.mutations()) {
        ...
        generateRow(row, keepIdentity, counter, i);
      } else {
        data[i] = oldData[i];
      }
    }
