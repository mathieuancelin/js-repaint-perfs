...
function databaseRenderer(database) {
  ...
  return spreadH(
    ...
    database.lastSample.topFiveQueries.map(function (query) {
      return vnodeImmutableThunk(queryRenderer.bind(_this, query), [query.elapsedClassName, query.formatElapsed, query.query], null, null, true);
    })
  );
}
        ...
        spreadH(
          ...
          spreadH(
            ...
            this.state.databases.map(function (database) {
              return vnodeImmutableThunk(databaseRenderer.bind(_this2, database), [database.lastMutationId], null, null, true);
            })
          )
        )
      );
...
function spreadH(tagName, properties) {
