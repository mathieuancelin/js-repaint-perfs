...
function VNode(tag, props, children, text, elm) {
  return {tag: tag, props: props, children: children, text: text, elm: elm};
}
...
function getKey(vnode) {
  return isUndef(vnode.props) ? undefined : vnode.props.key;
}

function sameVnode(vnode1, vnode2) {
  return isUndef(vnode1.key) ? vnode1.tag === vnode2.tag
                             : getKey(vnode1) === getKey(vnode2);
}
...
function updateChildren(parentElm, oldCh, newCh) {
  ...
  var oldStartIdx = 0, oldEndIdx = oldCh.length - 1;
  var newStartIdx = 0, newEndIdx = newCh.length - 1;
  var oldStartVnode = oldCh[0], oldEndVnode = oldCh[oldEndIdx];
  var newStartVnode = newCh[0], newEndVnode = newCh[newEndIdx];
  ...
  while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
    if (isUndef(oldStartVnode)) {
      ...
      var idxInOld = oldKeyToIdx[getKey(newStartVnode)];
...
},{"../oldsnabbdom.js":2,"../snabbdom.js":4,"benchmark":1}],4:[function(require,module,exports){
// jshint newcap: false
(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    define([], factory); // AMD. Register as an anonymous module.
...
function updateChildren(parentElm, oldCh, newCh) {
  ...
  while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
    if (isUndef(oldStartVnode)) {
      ...
      while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx &&
             !isUndef(oldEndVnode) && sameVnode(oldEndVnode, newEndVnode)) {
        patchElm(oldEndVnode, newEndVnode);
        oldEndVnode = oldCh[--oldEndIdx];
        newEndVnode = newCh[--newEndIdx];
      }
