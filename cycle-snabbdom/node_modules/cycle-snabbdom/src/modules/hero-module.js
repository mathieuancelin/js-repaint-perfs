...
function destroy(vnode) {
  ...
  if (hero && hero.id) {
    ...
    let computedStyle = window.getComputedStyle(elm, null); //get current styles (includes inherited properties)
  ...
  for (i = 0; i < created.length; i += 2) {
    ...
    if (oldVnode) {
      ...
      for (let key in oldVnode.savedStyle) { //re-apply saved inherited properties
        if (parseInt(key) != key) {
          let ms = key.substring(0, 2) === 'ms';
          let moz = key.substring(0, 3) === 'moz';
          let webkit = key.substring(0, 6) === 'webkit';
